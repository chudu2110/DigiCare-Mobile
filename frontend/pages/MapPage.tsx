...
export const MapPage: React.FC = () => {
  ...
  const evMarkerRefs = useRef<Record<number, any>>({});
  ...
  useEffect(() => {
    if (filters.includes(MapServiceType.CURRENT_EVENTS) && eventMarks.length > 0) {
      const m = eventMarks[0];
      if (mapRef.current) mapRef.current.setView([m.lat, m.lon], 15, { animate: true });
      const ref = typeof m.ts==='number' ? evMarkerRefs.current[m.ts] : null;
      if (ref && typeof ref.openPopup==='function') ref.openPopup();
    }
  }, [filters, eventMarks]);
  return (
          ...
          <div className="w-full lg:w-2/3 xl:w-3/4 bg-slate-100 dark:bg-slate-900 rounded-xl relative overflow-hidden">
            ...
            >
              ...
              {filters.includes(MapServiceType.CURRENT_EVENTS) && eventMarks.map((m:any, idx:number) => (
                <CircleMarker ref={(node)=>{ if(node && typeof m.ts==='number') evMarkerRefs.current[m.ts]=node; }} key={idx} center={[m.lat, m.lon]} radius={9} pathOptions={{ color: '#f97316', fillColor: '#f97316', fillOpacity: 0.6 }}>
                      ...
                      {eventMarks.length > 0 ? (
                        <div className="flex flex-col">
                          {eventMarks.map((m:any, idx:number) => (
                            <button key={idx} onClick={()=>{ setSelectedPoint(null); setSearchPos([m.lat, m.lon]); if(mapRef.current) mapRef.current.setView([m.lat, m.lon], 16, { animate: true }); const ref = typeof m.ts==='number' ? evMarkerRefs.current[m.ts] : null; if(ref && typeof ref.openPopup==='function') ref.openPopup(); }} className="w-full text-left px-3 py-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-md">
